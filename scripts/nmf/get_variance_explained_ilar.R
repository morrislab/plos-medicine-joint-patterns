# Calculates variance explained from the ILAR classifications.
...
global.r2s <- foreach(j = 2:ncol(dt.responses), .combine = 'c') %dopar% {
	...
	summary.res <- summary(lm(data.matrix(dt.responses[, j, with = FALSE]) ~ dt.predictors$diagnosis))

	summary.res$r.squared

}

dt.global <- data.table(diagnosis = -1, percent_variance_explained = mean(global.r2s) * 100)



message('Calculating per-classification variance explained')

all.clusters <- dt.predictors[, sort(unique(diagnosis))]
...
dt.var <- rbindlist(foreach(k = all.clusters) %dopar% {
	...
	all.r2s <- foreach(j = 2:ncol(dt.responses), .combine = 'c') %do% {
		...
		predictor.vec <- factor(dt.predictors$diagnosis == k)
	...
	dt.result <- data.table(diagnosis = k, percent_variance_explained = mean(all.r2s) * 100)
