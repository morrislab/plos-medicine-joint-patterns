...
parser$add_argument('--output', required = TRUE, help = 'the Feather file to output curve data to')
...
survfit.result <- survfit(Surv(duration, event_status) ~ rf + localization, dt.data)
...
extracted.strata <- str_match(dt.base$strata, '^rf=(.+),\\s+localization=(.+)$')

dt.base[, `:=`(
	rf = gsub('\\s', '', extracted.strata[, 2]),
	localization = gsub('\\s', '', extracted.strata[, 3]),
	strata = NULL
)]

dt.shifted <- dt.base %>% group_by(rf, localization) %>% do(make.shifted.data(.))
