# Plots hazard curves from the Cox regression.
...
parser$add_argument('--stratum', required = TRUE, help = 'the variable to stratify on')
...
pl <- ggplot(df.data, aes(x = time)) +
	facet_grid(as.formula(paste('. ~', args$stratum))) +
	...
	labs(x = 'Time (months)', y = 'P(non-zero involvement)') +
