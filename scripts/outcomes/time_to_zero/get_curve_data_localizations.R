...
survfit.result <- survfit(Surv(duration, event_status) ~ localization, dt.data)
...
extracted.strata <- str_match(dt.base$strata, '^localization=(.+)$')

dt.base[, `:=`(
	localization = gsub('\\s', '', extracted.strata[, 2]),
	strata = NULL
)]

dt.shifted <- dt.base %>% group_by(localization) %>% do(make.shifted.data(.))
