...
survfit.result <- survfit(Surv(duration, event_status) ~ classification + localization, dt.data)
...
extracted.strata <- str_match(dt.base$strata, '^classification=(.+),\\s+localization=(.+)$')
...
dt.shifted <- dt.base %>% group_by(classification, localization) %>% do(make.shifted.data(.))
