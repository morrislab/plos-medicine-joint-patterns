...
survfit.result <- survfit(Surv(duration, event_status) ~ diagnosis, dt.data)
...
extracted.strata <- str_match(dt.base$strata, '^diagnosis=(.+)$')

dt.base[, `:=`(
	diagnosis = gsub('\\s', '', extracted.strata[, 2]),
	strata = NULL
)]

dt.shifted <- dt.base %>% group_by(diagnosis) %>% do(make.shifted.data(.))
